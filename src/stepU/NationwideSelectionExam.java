 ...
 */
public class NationwideSelectionExam {
    ...
    public static int solution(int[] rank, boolean[] attendance) {
        ...
        // 등수대로 나열해 참여할 수 있는 학생의 번호를 a, b, c에 담는다.
        int[] selectArr = new int[3];
        int count = 0;
        for (int rankNum = 1; rankNum <= rank.length; rankNum++) {
            if (count == 3) break;
            for (int i = 0; i < rank.length; i++) {
                // rank_num 등수와 일치하고 참여하는 경우
                if (rank[i] == rankNum) {
                    if (attendance[i]) {
                        selectArr[count] = i;
                        count++;
        ...
        answer = 10000 * selectArr[0] + 100 * selectArr[1] + selectArr[2];
        return answer;
    }
}
